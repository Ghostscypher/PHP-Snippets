...
class EmailTemplateStringParser {
    ...
    /** Start of lower order replacement functions **/
    protected function lowerOrderReplacementsFunctionFor(){
    ...
    protected function processFunctionalReplacements(){
        preg_match_all('/{#[a-zA-Z0-9_]*:?[a-zA-Z0-9,]*}+/m', $this->processed_string, $function_placeholders);
    ...
    /**
     * Lower order replacements are the ones that should first be replaced
     * since they affect the template string structure.
     * 
     * For now we have the following lower order function
     *  1. for
     * 
     * in future we might add new functions 
     */
    protected function processLowerOrderReplacements(){
        preg_match_all('/{![a-zA-Z0-9_]*:[a-zA-Z0-9\-_, ]*}+/m', $this->processed_string, $lower_order_functions_placeholders);
        
        $lower_order_functions_placeholders = array_unique($lower_order_functions_placeholders[0] ?? []);

        foreach($lower_order_functions_placeholders as $lower_order_function){
            $function_name = trim($lower_order_function, '{#}');
            $temp = explode(":", $function_name);
            $function_name = $temp[0];
            array_shift($temp);
            $temp = implode('', $temp);
            ...
            // Set function name
            $function_name = sprintf("lowerOrderReplacementsFunction%s", $function_name);

            if(function_exists($function_name)){
                $this->processed_string = call_user_func($function_name, $this->processed_string, $args);
            } else if(method_exists(self::class, $function_name)){
                $this->processed_string = call_user_func([self::class, $function_name], $this->processed_string, $args);
            } else {
                throw new Exception(sprintf("Parse function %s not found", $lower_order_function));
            }
        }
    ...
    public function parse(){
        ...
        // NB. Lower order functions always start
        $this->processed_string = $this->processLowerOrderReplacements();
